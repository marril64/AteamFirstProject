...
public class Test3UpdateService implements Test3Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 1; i <= 10; i++) {
			...
			if (dao.test3Join(rId.get(i - 1), rName.get(i - 1), rGender.get(i - 1), rCountry.get(i - 1), rCity.get(i - 1))) {
		        out.write("<script>alert('" + "가입이 승인되었습니다." + "');location.href='" + "http://localhost:8181/AteamFirstProject/firstProject/test3.test3" + "';</script>");
		        out.flush();
		        out.close();
			} else {
		        out.write("<script>alert('" + rId.get(i - 1) + "는 중복된 아이디입니다." + "');location.href='" + "http://localhost:8181/AteamFirstProject/firstProject/test3.test3" + "';</script>");
		        out.flush();
		        out.close();
			}
		}
		...
		if (request.getParameter("tNum") != null) {
			...
			if (dao.test3Update(tNum, tId, tName, tGender, tCountry, tCity)) {
