	...
	<form action="#" method="post">
		...
		<div class="row">
			...
			<div class="col">
				...
				<div class="form-check form-check-inline">
		  			<input class="form-check-input" type="radio" name="tGender" id="inlineRadio1" value="남" OnClick="window.location.href='/AteamFirstProject/firstProject/gender.test3?tGender=남';">
		  			<label class="form-check-label" for="inlineRadio1">남</label>
				</div>
				<div class="form-check form-check-inline">
		  			<input class="form-check-input" type="radio" name="tGender" id="inlineRadio2" value="여" OnClick="window.location.href='/AteamFirstProject/firstProject/gender.test3?tGender=여';">
		  			<label class="form-check-label" for="inlineRadio2">여</label>
				</div>
				<div class="form-check form-check-inline">
		  			<input class="form-check-input" type="radio" name="tGender" id="inlineRadio3" value="회원 전체보기" OnClick="window.location.href='/AteamFirstProject/firstProject/gender.test3?tGender=회원 전체보기';">
		...
		<div class="btn-group" role="group" aria-label="Basic outlined example">
			<button type="button" class="btn btn-outline-primary">
				<input type="submit" value="조회" formaction="/AteamFirstProject/firstProject/search.test3">
			</button>
			<button type="button" class="btn btn-outline-primary">
				<input type="button" value="추가" onclick="rowUp()">
	  		</button>
	  		<button type="button" class="btn btn-outline-primary">
				<input type="submit" value="저장" onclick="return up()" formaction="/AteamFirstProject/firstProject/update.test3">
	  		</button>
			<button type="button" class="btn btn-outline-primary">
				<input type="submit" value="엑셀 다운로드" onclick="exportTableToExcel('tblData', 'test1Excel')" formaction="/AteamFirstProject/firstProject/excel.test3">
			</button>
	  		<button type="button" class="btn btn-outline-primary">
		  		<input type="submit" value="삭제" onclick="return test()" name="selected" formaction="/AteamFirstProject/firstProject/delete.test3">
	  		</button>
	  	</div>
		...
		<table class="table table-hover" id="tblData" border=1>
			...
			<tbody id="tBody">
				...
				<c:if test="${list.tGender eq '남'}">
					<tr ondblclick="dbRow('${list.tNum}', '${list.tId}', '${list.tName}', '${list.tGender}', '${list.tCountry}', '${list.tCity}');" class="table-primary">
						<td>
				...
				<c:if test="${list.tGender eq '여'}">
					<tr ondblclick="dbRow('${list.tNum}', '${list.tId}', '${list.tName}', '${list.tGender}', '${list.tCountry}', '${list.tCity}');" class="table-danger">
						<td>
							<input type="checkbox" name="tNum" class="tNum" value="${list.tNum}" id="tNum">
						</td>
						<td id="tId">${list.tId}</td>
...
﻿ ﻿ 
	...
	var subOptions = {
			none : ["국가를 선택해주십시오."],
			korea : ["전체", "서울", "인천", "대구", "광주", "부산", "전주"],
			usa : ["전체", "워싱턴", "뉴욕", "로스엔젤러스"],
			japan : ["전체", "도쿄", "오사카", "히로시마"]
	}
	var subOption = [];
	...
	mainCountry.onchange = function () {
		...
		for (let i = 0; i < subOption.length; i++) {
			...
			option.innerHTML = '<label for="one"><input type="checkbox" value="' + subOption[i] + '"/>' + subOption[i] + '</label>';
			subCity.append(option);
		}
		﻿
	}
    ...
    function dbRow(num, id, name, gender, country, city) {
    	...
    	var windowStatus = 'width='+width+', height='+height+', left='+left+', top='+top+', scrollbars=yes, status=yes, resizable=yes, titlebar=yes';
        	const url = "http://localhost:8181/AteamFirstProject/firstProject/test3Popup.jsp?tNum=" + num + "&tId=" + id + "&tName=" + name + "&tGender=" + gender + "&tCountry=" + country + "&tCity=" + city;
        	openWin = window.open(url, "Test3", windowStatus);
    }
   ...
   /* CREATE TABLE test2 (
        ...
        tGender varchar2(20) not null,
        tCountry varchar2(20) not null,
        tCity varchar2(20) not null,
        tJoin date default sysdate
    );

    drop table board_tbl;

    CREATE SEQUENCE testNum START WITH 1 INCREMENT BY 1 MAXVALUE 1000 CYCLE NOCACHE;


    insert into test3 (tNum, tId, tName, tGender, tCountry, tCity) values (testNum.nextval, 'marril64', '신동호', '남', '대한민국', '서울');
    insert into test3 (tNum, tId, tName, tGender, tCountry, tCity) values (testNum.nextval, 'tkrhd123', '사공율', '남', '대한민국', '부산');
    insert into test3 (tNum, tId, tName, tGender, tCountry, tCity) values (testNum.nextval, 'zldqpf12', '김종훈', '남', '대한민국', '대구');
